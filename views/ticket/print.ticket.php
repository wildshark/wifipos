<?php
/**
 * Created by PhpStorm.
 * User: Andrew Quaye
 * Date: 30-Oct-17
 * Time: 9:05 AM
 */

$id=$_SESSION['package'];

$package="select * from get_ticket_4sell WHERE bundle_type='$id'";
$result=$conn->query($package);
$r=$result->fetch_assoc();

$ticket_id=$r['pinID'];

$_SESSION['date']=$r['create_date'];
$_SESSION['username']=$r['username_id'];
$_SESSION['password']=$r['password_id'];
$_SESSION['status']=$r['status'];
$user=$_SESSION['user_id'];
$now=date('Y-m-d H:i:s');

$sql="UPDATE `ticket_bank` SET `userid`='$user', sold_date='$now',`status`='2' WHERE (`pinID`='$ticket_id') LIMIT 1";
$result=$conn->query($sql);

if ($result === TRUE){
    $username=$_SESSION['username'];
    $password= $_SESSION['password'];

    function qr_code($username,$password){
        $size="150";
        $url="http://hotspot.local/login?username='$username'&password='$password'";
        return'<img src="https://api.qrserver.com/v1/create-qr-code/?size='.$size.'x'.$size.'&data='.$url.'"/>';
    }
}

?>
<!DOCTYPE html>
<html>
<head>
    <title>HTML to API - event tickets</title>
    <link href='https://fonts.googleapis.com/css?family=Open+Sans:400,700|Lobster|Kreon:700&subset=latin,latin-ext' rel='stylesheet' type='text/css'>
    <!-- <link rel="stylesheet" href="styles/main.css" media="screen" charset="utf-8"/> -->
    <meta content="width=device-width, initial-scale=1.0" name="viewport">
    <meta http-equiv="content-type" content="text-html; charset=utf-8">
</head>

<style>
    body {
        margin: 0;
        color: #ffffff;
        font-family: "Open Sans", sans-serif;
        font-weight: 400;
        font-size: 24px;
    }

    .container {
        width: 795px;
        margin: 0 auto;
    }

    section {
        -moz-box-sizing: border-box;
        -webkit-box-sizing: border-box;
        box-sizing: border-box;
        height: 280px;
        padding: 0 60px;
        background-image: url(data:image/svg+xml;charset=utf-8;base64,);
        background-repeat: no-repeat;
        overflow: hidden;
    }
    section .title {
        float: left;
        padding: 22px 65px 23px;
        margin-bottom: 42px;
        background-color: #ffffff;
        box-shadow: 0px 11px 9px 0px rgba(0, 0, 0, 0.5);
        color: #984AD1;
        font-family: "Lobster", cursive;
        font-size: 2.48em;
    }
    section .event {
        width: 50%;
        float: right;
        padding-top: 15px;
        font-family: "Open Sans", sans-serif;
        font-weight: 700;
        font-size: 1em;
        text-align: right;
        line-height: 55px;
        text-transform: uppercase;
    }
    section .info {
        clear: both;
        margin-bottom: 40px;
        text-transform: uppercase;
    }
    section .seats {
        display: inline-block;
        margin-right: 150px;
        font-size: 0.375em;
        text-transform: uppercase;
        vertical-align: middle;
    }
    section .seats:last-child {
        margin-right: 0;
    }
    section .seats span {
        position: relative;
        font-family: "Kreon", serif;
        background-color: #fff;
        color: #984AD1;
        padding: 10px 15px;
        margin-left: 10px;
        font-size: 2.666em;
        box-shadow: 0px 8px 8px 4px rgba(0, 0, 0, 0.5);
        vertical-align: middle;
    }
</style>

<body>
<div class="container">
    <section>
        <div class="title"><?php echo qr_code($username,$password);?></div>
        <div class="event">Username:<?php echo $_SESSION['username'];?></div>
        <div class="event">Password:<?php echo $_SESSION['password'];?></div>
        <div class="info">Ghana CU Internet Bundle </div>
    </section>
</div>
<a href="page.php?page=ticket&alert-msg=0"><button>Finish</button></a>
</body>

</html>
